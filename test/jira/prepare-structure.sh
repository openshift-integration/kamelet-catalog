...
kubectl create secret generic conn-credentials --from-file=conn-credentials.properties -n ${YAKS_NAMESPACE}

# bind secret to jira-source test
kubectl label secret conn-credentials yaks.citrusframework.org/test=jira-source -n ${YAKS_NAMESPACE}
